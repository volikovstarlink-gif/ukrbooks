      ...
      {data && !loading && (
        <>
          ...
          <div className="bg-[#1e293b] rounded-2xl p-6 border border-white/10">
            <h3 className="font-semibold mb-4 text-slate-200">📊 По мережах</h3>
                    ...
                    {data.networks.map(n => {
                      ...
                      return (
                        <tr key={n} className="border-b border-white/5 hover:bg-white/5">
                          <td className="py-2 pr-4 text-slate-200 font-medium">{n}</td>
                          <td className="text-right py-2 pr-4 text-blue-400">{s.impressions.toLocaleString('uk-UA')}</td>
                          <td className="text-right py-2 pr-4 text-emerald-400">{s.clicks.toLocaleString('uk-UA')}</td>
                          <td className="text-right py-2 pr-4 text-cyan-400">{s.ctr.toFixed(2)}%</td>
                          <td className={`text-right py-2 ${s.errors > 0 ? 'text-red-400' : 'text-slate-500'}`}>
                            {s.errors.toLocaleString('uk-UA')}
                          </td>
                        </tr>
                      );
                    })}
